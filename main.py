...
class App(tk.Tk):
    def __init__(self):
        ...
        self.tab_mib_sozlamalar = SozlamalarGuruh(
            self.mib_inner, self, "MIB sozlamalari", MIB_SOZLAMA_FIELDS, sections=[])
...
class MijozHolatiDialog(tk.Toplevel):
    ...
    def _bosqich_qatori(self, parent, nomi, holat_matni, tag=None):
        row = tk.Frame(parent, bg=WHITE)
        ...
        tk.Label(row, text=holat_matni, bg=WHITE, fg=color, font=('Segoe UI', 9),
                 anchor='w', wraplength=380, justify='left').pack(side='left', pady=4)
    ...
    def _build_kartochka(self, parent, item, idx, jami):
        ...
        if xat.get('mib_holati') == 'otkazildi':
            ...
            self._bosqich_qatori(card, "4) MIB:", mib_matn, tag='ok')
            if oxirgi_mib:
