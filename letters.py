...
def generate_yigma_jild_titul(output_path, xat, portfel_row, mijoz, settings):
    """
    Ijro harakati (MIB) yig'ma jildi uchun titul (muqova) hujjatini yaratadi.
    Bu qog'oz holatidagi ish papkasining birinchi varag'i bo'lib, ish bo'yicha
    barcha asosiy ma'lumotlarni bir joyda ko'rsatadi.
    """
    ...
    run = p.add_run(settings.get('bank_nomi', ''))
    run.bold = True
    run.font.size = Pt(12)

    p2 = doc.add_paragraph()
    p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run2 = p2.add_run(f"{settings.get('filial_nomi', '')} филиали")
    ...
    trun = title.add_run("ИЖРО ҲАРАКАТИ\nЙИҒМА ЖИЛДИ")
    trun.bold = True
    ...
    srun = sub.add_run(f"Иш рақами: {xat.get('mib_ish_raqami', '')}")
    ...
    def qator(label, val):
        ...
        row.cells[1].text = str(val) if val not in (None, '') else '—'
        for cell in row.cells:
    ...
    qator("Қарздор", mijoz_ism)
    qator("Тури", "Юридик шахс" if xat.get('mijoz_turi') == 'yuridik' else "Жисмоний шахс")
    qator("Манзили", mijoz_manzil)
    qator("Ҳужжат рақами (ПИНФЛ/СТИР)", hujjat_raqami)
    qator("Анкета рақами", xat.get('anketa_raqami', ''))
    qator("Жами қарздорлик (сўм)", f"{jami_qarz:,.0f}".replace(',', ' '))
    qator("", "")
    qator("Огоҳлантириш/Талабнома хати санаси",
          _fmt_sana_iso(xat.get('yaratilgan_sana')))
    qator("Хат юборилган сана", _fmt_sana_iso(xat.get('yuborilgan_sana')))
    qator("Даъво ариза тайёрланган сана", _fmt_sana_iso(xat.get('davo_ariza_sana')))
    qator("Палатадан қайтган сана", xat.get('davo_ariza_imzo_sana', ''))
    qator("Судга топширилган сана", xat.get('sud_topshirilgan_sana', ''))
    qator("Суд иши рақами", xat.get('sud_ish_raqami', ''))
    qator("", "")
    qator("МИБга ўтказилган сана", xat.get('mib_otkazilgan_sana', ''))
    qator("МИБ иш рақами", xat.get('mib_ish_raqami', ''))
    qator("Жилд очилган сана", datetime.date.today().strftime('%d.%m.%Y'))

    doc.add_paragraph()
    doc.add_paragraph()
    footer = doc.add_paragraph()
    footer.alignment = WD_ALIGN_PARAGRAPH.CENTER
    frun = footer.add_run("Ушбу жилдга барча тегишли ҳужжатлар (ижро вараqаси, далолатнома, "
                           "сотув ҳужжатлари ва бошқалар) тартиб билан илова қилиб борилади.")
    frun.italic = True
    frun.font.size = Pt(9)
